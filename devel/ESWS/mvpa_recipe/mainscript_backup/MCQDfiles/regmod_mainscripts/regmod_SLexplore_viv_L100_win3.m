...
config_options.summed_behavior4regmod = 'off';
config_options.crossval_method = 'runwise'; %runwise
config_options.TRlag = 1;%TR lag
config_options.TR_avg_window = 3;%TR window to average over
config_options.classifier = @minpool;
config_options.result_dir = aname;


options = set_options(config_options);
options.which_behavior = 2; %only specify for complete_file behavioral measure
options.searchlight_radius = 1;

brain_cells = regmod_explore(options);
