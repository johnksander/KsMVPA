...
for idx = 1:numel(options.subjects)
    if ismember(options.subjects(idx),options.exclusions) == 0
        ...
        disp(sprintf('\nNormalizing subject %g fMRI data',options.subjects(idx)))
        ...
        for roi_idx = 1:numel(options.roi_list)
            ...
            save(preproc_data_file_pointers{idx,roi_idx},'preprocessed_scans'); %save ROI so we don't have to preprocess next time 'round
