function model_deviations = RSA_roi_WinSub_bootstrap_mdl(subject_file_pointers,options)

%adopted from RSA_roi_latecomb_bootstrap_mdl() for within subject use...
...
%   4. Test RDMs with bootstrapped trials 
...
model_deviations = NaN(numel(options.subjects),options.num_straps);
...
for roi_idx = 1:numel(options.roi_list)
    ...
    for subject_idx = 1:numel(options.subjects)
        if ismember(options.subjects(subject_idx),options.exclusions) == 0
            ...
            brainRSM(logical(eye(size(brainRSM)))) = NaN;
            stim_RSM(logical(eye(size(stim_RSM)))) = NaN;
    
            disp('--------------------------------------------------------------------')
            disp('Calculating subject model deviations via bootstrap resampling stimuli')
            
            %   4. Test RDMs with bootstrapped trials
            %preallocate bootstrap samplings
            num_trials = numel(subject_stims);
            strapped_samples = randi(num_trials,num_trials,options.num_straps);
            for strapidx = 1:options.num_straps
                if mod(strapidx,2500) == 0
                    disp(sprintf('----sample #%i/%i complete',strapidx,options.num_straps))
                end
                curr_sample = strapped_samples(:,strapidx);
                %RSA_permuteRDM() will work with resampling as well, same code applies  
                strapped_brain_RSM = RSA_permuteRDM(brainRSM,curr_sample);
                strapped_brain_RSM = strapped_brain_RSM(:); 
                strapped_model_RSM = RSA_permuteRDM(stim_RSM,curr_sample);
                strapped_model_RSM = strapped_model_RSM(:);
                model_deviations(subject_idx,strapidx) = ...
                    corr(strapped_brain_RSM,strapped_model_RSM,'type','Spearman','rows','pairwise');
            end
