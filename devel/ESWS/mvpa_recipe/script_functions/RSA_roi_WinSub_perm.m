function permuted_subject_fits = RSA_roi_WinSub_perm(subject_file_pointers,options)

%adapted from RSA_roi_latecomb() for within subject fits (permuted here)
...
%   4. set permutation order
%   5. Test permuted RDMs
...
permuted_subject_fits = NaN(numel(options.subjects),options.num_perms); %fits
    ...
    for subject_idx = 1:numel(options.subjects)
        if ismember(options.subjects(subject_idx),options.exclusions) == 0
            ...
            for permidx = 1:options.num_perms
                if mod(permidx,2500) == 0
                    fprintf('----RSM #%i/%i complete\n',permidx,options.num_perms)
                end
                perm_order = randperm(numel(subject_stims))';
                %permute the stimuli matrix
                permutedRSM = RSA_permuteRDM(stim_RSM,perm_order);
                permutedRSM = permutedRSM(mat2vec_mask); %take the upper triangular
                
                %   5. Test permuted RDMs
                switch options.RDM_dist_metric
                    case 'spearman'
                        permuted_subject_fits(subject_idx,permidx) = ...
                            corr(brainRSM,permutedRSM,'type','Spearman');
                end
            end
        end
    end
