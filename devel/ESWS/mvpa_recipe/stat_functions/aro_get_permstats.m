function aro_get_permstats(roi_statistics,roi_output,options)

rois = options.rois4fig;

num_perms = 100000;
ci_range = 90;
behaviors = 1:numel(roi_output(1,1,:));
...
for x = 1:size(roi_output,1),%subj
    for y = 1:size(roi_output,2),%rois
        for z = 1:size(roi_output,3), %behavior conds
            if ~isempty(roi_output{x,y,z}) & numel(unique(roi_output{x,y,z})) > 1 %condition has all trials of one label...
            ...
            elseif numel(unique(roi_output{x,y,z})) == 1
                disp(sprintf('subj %i : found instance of condition with only one trial type',x))
            end
        end
    end
end
...
save(fullfile(options.save_dir,options.permstats_fname),'acc_means','acc_p_values','acc_ci','mcc_means','mcc_p_values','mcc_ci');
