function label_predictions = GNB(cv_params,options)
...
class_priors = NaN(size(class_labels)); %got a record so they put me with the baddest bunch
for idx = 1:numel(class_labels)
    ...
    class_priors(idx) = log(sum(curr_class) / numel(curr_class));
end
...
for idx = 1:numel(class_labels)
    ...
    %voxel joint probability with class priors
    test_class_probs(:,idx) = sum(test_data,2) + class_priors(idx);
end
