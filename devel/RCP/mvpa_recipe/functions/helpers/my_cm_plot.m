function out1 = my_cm_plot(varargin)
...
plotData.CONTROL.text = uicontrol('Parent',fig,'Style','text',...
    ...
    'FontWeight','bold','ForegroundColor',[0.7 0 0]); %red background
set(fig,'UserData',plotData);
end
...
PTFS = nnplots.title_font_size;
trainColor = [0 0 1]; %blue 
valColor = [0 1 0]; %green
testColor = [1 0 0]; %red
                ...
                for j=1:numColumns
                    for k=1:numColumns
                        ...
                        elseif (j<numColumns) && (k<numColumns)
                            ...
                            bottombold = 'normal';
                        elseif (j<numColumns)
                            %c = [0.5 0.5 0.5]; %this is that shitty gray 
                            c = [1,1,1]; %normal ppl color 
                            topcolor = [0 0 0];
                            bottomcolor = [0 0 0];
                            topbold = 'normal';
...
for i=1:numSignals
    ...
    for j=1:numColumns
        for k=1:numColumns
            if (j==numColumns) && (k==numColumns)
                %This is total accuracy, remove bottom 
                correct = sum(diag(cm));
                perc = correct/numSamples;
                top = percent_string(perc);
                %get rid of this bottom
                %bottom = percent_string(1-perc);
            elseif (j==k)
                %this is fine, cells for correct labels 
                num = cm(j,k);
                top = num2str(num);
                perc = num/numSamples;
                bottom = percent_string(perc);
            elseif (j<numColumns) && (k<numColumns)
                %this is fine, cells for misclassified labels 
                num = cm(j,k);
                top = num2str(num);
                perc = num/numSamples;
                bottom = percent_string(perc);             
            elseif (j<numColumns)
                %this is specificity, remove bottom 
                correct = cm(j,j);
                total = sum(cm(j,:));
                perc = correct/total;
                top = percent_string(perc);
                ...
                %this is precision, remove bottom 
                correct = cm(k,k);
                total = sum(cm(:,k));
                perc = correct/total;
                top = percent_string(perc);
            ...
            %control labeling here 
            %K is rows, j is columns??
            if k == 1 && j == numColumns
                bottom = sprintf('precision\n(TP / TP + FP)');
            elseif k == 2 && j == numColumns
                bottom = sprintf('negative predictive value\n(TN / TN + FN)');
            elseif k == numColumns && j == 1
                bottom = sprintf('sensitivity\n(TP / TP + FN)');
            elseif k == numColumns && j == 2
                bottom = sprintf('specificity\n(TN / TN + FP)');
            elseif k == numColumns && j == numColumns
                bottom = 'accuracy';
            end
