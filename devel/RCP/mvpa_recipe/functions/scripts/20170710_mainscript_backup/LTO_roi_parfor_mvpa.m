function predictions = LTO_roi_parfor_mvpa(subject_file_pointers,options)
...
%make all combinations of 5 US subjects & 5 EA subjects
LTOcombos = leave_ten_out_combos(options);
predictions = cell(numel(LTOcombos(1,:)),numel(options.roi_list),numel(options.behavioral_file_list));
...
for roi_idx = 1:numel(options.roi_list)
    ...
    for beh_idx = 1:numel(options.behavioral_file_list) %Cycle through valence levels, performing classification on each
        ...
        parfor test_sub_combo = 1:numel(LTOcombos(1,:)) 
            ...
            testing_subject = LTOcombos(:,test_sub_combo);
