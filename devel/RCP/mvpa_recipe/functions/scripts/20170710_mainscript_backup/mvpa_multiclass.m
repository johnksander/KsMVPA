function [predictions,auc_matrix] = mvpa_multiclass(subject_file_pointers,options)
%GOALS:::
...
%                     cv_params.fe_testing_data = extract_brain_features(cv_params.testing_data,trained_centroids,M,P);
                    ...
                    [cv_guesses{cv_idx},aucs(cv_idx)] = options.classifier_type(cv_params,options);
                end
                cv_guesses = cat_cv_inds(cv_guesses);
                predictions{idx,roi_idx,beh_idx} = cat(1,cv_guesses{:});
		auc_matrix(idx,roi_idx,beh_idx) = mean(aucs);
            end
