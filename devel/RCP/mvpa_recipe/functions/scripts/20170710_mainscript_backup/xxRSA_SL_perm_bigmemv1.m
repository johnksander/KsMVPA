...
%0. Initialize variables
voxel_null = cell(numel(options.roi_list),options.num_perms);
...
for roi_idx = 1:numel(options.roi_list)
    ...
    for permidx = 1:options.num_perms
        update_logfile(['Starting permutation number ' num2str(permidx)],output_log)
        permuted_order = randperm(num_conds)'; %set permutation order
        
        %7. slice searchlights & assemble RDM
        searchlight_results = NaN(numel(seed_inds),1);%easier results-to-roi file mapping
        parfor searchlight_idx = 1:numel(seed_inds) %parfor
            switch options.parforlog %parfor progress tracking
                case 'on'
                    ...
                    if mod(sum(progress),numel(seed_inds) * .005) == 0 %.5 percent
                        ...
                        message = sprintf('Permutation number %i-- %.1f percent complete',permidx,progress);
                        update_logfile(message,output_log)
                    end
            end
            ...
            for subject_idx = 1:sum(valid_subs) %all exclusions already taken care of
                ...
                [data_matrix,CVbeh_data] = select_trials(data_matrix,CVbeh_data{subject_idx});%select trials
            ...
            current_searchlight = current_searchlight(permuted_order,:); %permute conditions
            ...
            searchlight_results(searchlight_idx) = corr(RDM,Hmat,'type','Spearman','tail','right');
        end%searchlight parfor loop
        voxel_null{roi_idx,permidx} = searchlight_results;
    end
