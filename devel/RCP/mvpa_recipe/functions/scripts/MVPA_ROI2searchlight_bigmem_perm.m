function MVPA_ROI2searchlight_bigmem_perm(options)

error('unfinished!') %I left off at the parfor loop initialization 
...
%   9b. PCA feature selection & LDA training: searchlight matrix
    ...
    for subject_idx = 1:numel(options.subjects)
        if ismember(options.subjects(subject_idx),options.exclusions) == 0
            ...
            parfor permidx = 1:options.num_perms
                ...
                permuted_labels = permCVbeh_data(:,permidx);
                %   8. set up cross validation
                ...
                for fold_idx = 1:CV.kfolds %CV folds
                    ...
                    %class labels
                    training_labels = permuted_labels(training_logical);
                    testing_labels = permuted_labels(testing_logical);
                    ...
                    update_logfile(sprintf('----N PCs = %i',options.PCAcomponents2keep),output_log)
                    testing_searchlights = NaN(numel(training_labels),options.PCAcomponents2keep,numel(seed_inds));
                    ...
                    training_labels = repmat(training_labels,1,num_encROIs);
                    %pay attention! above I just expanded training labels for
                    ...
                    for searchlight_idx = 1:numel(seed_inds)
                        ...
                        predictions = NaN(numel(testing_labels),num_encROIs);
                        for enc_idx = 1:num_encROIs
                            encoding_model = ROI_models{enc_idx};
                            predictions(:,enc_idx) = predict(encoding_model,testing_data);
                        end
                        
                        %this shit needs to be fixed as well
                        
                        %   11. store CV accuracy
                        switch options.performance_stat
                            case 'accuracy'
                                searchlight_correct(searchlight_idx,:) = sum(predictions == training_labels); %count number correct
