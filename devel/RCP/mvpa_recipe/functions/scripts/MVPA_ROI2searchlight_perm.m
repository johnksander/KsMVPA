function MVPA_ROI2searchlight_perm(options)
...
%   9a. PCA feature selection & LDA training: encoding ROIs
%   9b. PCA feature selection: searchlight matrix
%   10. slice searchlights & classify
...
%Begin loops
update_logfile(':::Starting ROI-to-searchlight MVPA permutation testing:::',output_log)
    ...
    for subject_idx = 1:numel(options.subjects)
        if ismember(options.subjects(subject_idx),options.exclusions) == 0
            ...
            for fold_idx = 1:CV.kfolds %CV folds
                %store CV accuracy
                searchlight_correct = NaN(numel(seed_inds),options.num_perms,num_encROIs); %N correct label guesses for searchlight CV
                ...
                %   9a. Encoding ROI: PCA feature selection & LDA training
                ...
                %train models for all permuted orderings here
                ROI_models = cell(num_encROIs,options.num_perms);
                for enc_idx = 1:num_encROIs
                    ...
                    update_logfile(message,output_log)
                    %train LDA model on features with permuted orderings
                ...
                update_logfile('Classifying searchlights with permuted labelings',output_log)
                %   10. slice searchlights & classify with permuted labels
                ...
                parfor searchlight_idx = 1:numel(seed_inds)
                    ...
                    %classify with each ROI & permutation order
                    switch options.performance_stat
                        case 'accuracy'
                            predictions = NaN(sum(testing_logical),options.num_perms,num_encROIs);
                            for enc_idx = 1:num_encROIs
                                for permidx = 1:options.num_perms
                                    predictions(:,permidx,enc_idx) = ...
                                        predict(ROI_models{enc_idx,permidx},testing_data);
                                end
                            end
                            %   11. store CV accuracy
                            searchlight_correct(searchlight_idx,:,:) = sum(predictions == testing_labels); %count number correct
                            searchlight_guesses(searchlight_idx) = size(predictions,1); %count number of guesses
                        case 'oldMC'
                            predictions = NaN(size(testing_labels)); %expanded to (labels x class x perm x roi)
                            for enc_idx = 1:num_encROIs
                                for permidx = 1:options.num_perms
                                    predictions(:,:,permidx,enc_idx) = ...
            ...
            cv_correct = cat(4,cv_correct{:}); %put CV in 4th dim (since already SL x p x roi)
            cv_guesses = cat(2,cv_guesses{:}); %guesses constant across p & roi
            
            switch options.performance_stat
                case {'accuracy','oldMC'}
                    %mean accuracy across CV folds (total CV correct / total CV guesses)
                    cv_correct = sum(cv_correct,4); %sum across folds
