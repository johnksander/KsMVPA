function predictions = MVPA_ROI_enc2ret(subject_file_pointers,options)
...
predictions = cell(numel(options.subjects),numel(options.behavioral_file_list));
...
for subject_idx = 1:numel(options.subjects)
    if ~ismember(options.subjects(subject_idx),options.exclusions)
        ...
        for fold_idx = 1:CV.kfolds %CV folds
            ...
            switch options.feature_selection
                case 'martinez PCA'
                    ...
                    update_logfile(sprintf('---encoding var retained = %.2f percent',Einfo.variancePercent),output_log)
                    update_logfile(sprintf('---retrieval var retained = %.2f percent',Rinfo.variancePercent),output_log)
                otherwise
        ...
        predictions{subject_idx,beh_idx} = cv_guesses;
