...
hrf = spm_hrf(1); % 2 is TR time(s), upsampled to 1 second here HARDCODED
trialidx = trialidx:trialidx + (options.trial_length - 1); %also adjusting for upsampling...

%make regressor impulse functions
num_reg = numel(options.trialtypes) + 2;
TOI_type = unique(trial_onsets(trial_inds(trialidx))); %unique on this line added b/c trialidx is a vec now (for upsampling..) 
...
notrial_regs = sum(Xt) == 0;
if sum(notrial_regs) > 0 %a regressor without any trials ended up in here (usually encoding runs w/ no korn trials)
    Xt = Xt(:,~notrial_regs);
end
