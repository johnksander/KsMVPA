function softmaxModel = softmax_train(cv_params,options)

n = numel(cv_params.training_labels); %number of observations -- needs to be replaced if not simulating, e.g. n = size(behavior,1);
c = numel(cv_params.training_data(1,:)) + 1; %number of design matrix columns + 1 intercept
...
options.MaxIter = 500;
softmaxModel = softmaxTrain(c, cv_params.max_val, lambda, ...
                            X', y, options);
                        
function [softmaxModel] = softmaxTrain(input_size, num_classes, lambda, input_data, labels, options)
...
theta = 0.005 * randn(num_classes,input_size);
...
opt_theta = fminlbfgs( @(p) softmaxCost(p, ...
                                   num_classes, input_size, lambda, ...
                                   input_data, labels), ...                                   
                              theta, options);

% Fold softmaxOptTheta into a nicer format
softmaxModel.opt_theta = reshape(opt_theta, num_classes, input_size);
softmaxModel.input_size = input_size;
softmaxModel.num_classes = num_classes;
                          
function [cost, grad] = softmaxCost(theta, num_classes, input_size, lambda, data, labels)
