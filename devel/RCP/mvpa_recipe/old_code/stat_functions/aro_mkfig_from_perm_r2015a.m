function aro_mkfig_from_perm_r2015a(options)
...
%%
% mkfig config
label_beh = {'Pos Arousing' 'Pos Non-Arousing' 'Neutral' 'Neg Arousing' 'Neg Non-Arousing'};
label_roi = {'Hipp','Hipp', 'Amyg', 'Amyg', 'PhG', 'PhG'};

collist = hsv(50);
col = collist([1 5 22 32 28],:);
...
for beh = (1:length(label_beh))
    for roi = 1:numel(label_roi)
        ...
        errorbar_tick(e,125);
        if pval(roi,beh)<.05
        text(beh + (roi-1)*(numel(label_beh)  +1), acc_m(roi,beh)+acc_ci(roi,beh)+.01,sprintf('%s','*'),'horizontalalignment','center','fontsize',signum_fontsz);
        elseif pval(roi,beh)<.1
        text(beh + (roi-1)*(numel(label_beh)  +1), acc_m(roi,beh)+acc_ci(roi,beh)+.01,sprintf('%s','t'),'horizontalalignment','center','fontsize',(signum_fontsz - 12));
        end
    end
end
...
legend(label_beh, 'orientation', 'horizontal','location','northoutside','FontSize',12,'FontWeight','bold')
legend boxoff
...
text(10,.625,'Left','fontsize',30,'fontweight','b','horizontalalignment','center');
text(25,.625,'Right','fontsize',30,'fontweight','b','horizontalalignment','center');
...
set(gcf, 'PaperPosition', [.02 .1 .96 .8])
pause(2)
