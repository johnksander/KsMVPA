function jk_mkfig_4MCC_from_perm_r2015a(options)
...
%% 1. MCC
data = perm.mcc_means(:,hemisort);
perm.mcc_ci = perm.mcc_ci(:,hemisort,:);
perm.mcc_p_values = perm.mcc_p_values(:,hemisort);
...
for roi = 1:numel(label_roi)
    for beh = (1:length(label_beh))
        % descriptive stats
        mcc_m(roi,beh) = data(beh,roi);
        mcc_ci(roi,beh) = perm.mcc_ci(2,roi,beh);
        
        % inferential stats
        pval(roi,beh) = perm.mcc_p_values(beh,roi);
    end
end
...
for roi = 1:numel(label_roi)
    for beh = (1:length(label_beh))
        bar( beh + (roi-1)*(numel(label_beh) +1 ), mcc_m(roi,beh),'facecolor',col(beh,:),'linewidth',2);
    end
end

for beh = (1:length(label_beh))
    for roi = 1:numel(label_roi)
        e=errorbar(beh + (roi-1)*(numel(label_beh)  +1), mcc_m(roi,beh),mcc_ci(roi,beh),'k','linewidth',2);
        
        errorbar_tick(e,125);
        if pval(roi,beh)<.05
        text(beh + (roi-1)*(numel(label_beh)  +1), mcc_m(roi,beh)+mcc_ci(roi,beh)+.01,sprintf('%.3f',pval(roi,beh)),'horizontalalignment','center','fontsize',signum_fontsz);
        elseif pval(roi,beh)<.1
        text(beh + (roi-1)*(numel(label_beh)  +1), mcc_m(roi,beh)+mcc_ci(roi,beh)+.01,sprintf('%.3f',pval(roi,beh)),'horizontalalignment','center','fontsize',(signum_fontsz - 4));
        end
    end
end
...
ylabel('Matthew''s Corr. Coef.','linewidth',2,'fontsize',fontsz);
...
print(fullfile(options.save_dir,[options.figure_fname '_mcc_' datestr(now,29) '.pdf']),'-dpdf')
