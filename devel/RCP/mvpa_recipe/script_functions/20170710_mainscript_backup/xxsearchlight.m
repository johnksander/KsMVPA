function brain_cells = xxsearchlight(options)
...
brain_cells = cell(numel(options.subjects),2);
...
for idx = 1:numel(options.subjects),
    if ismember(options.subjects(idx),options.exclusions) == 0,
        ...
        normed_data = normalize_scan_data(file_data); %detrend & zscore data runwise + account for HDR
        ...
        for il = 1:numel(searchlight_inds(1,:)),
            ...
            if sum(isnan(searchlight_inds(:,il))) == 0, %run if entire searchlight is in the brain
                ...
                output_brain(seed_x,seed_y,seed_z,beh_idx) = NaN;
            end
        end
        brain_cells{idx,1} = output_brain;
        brain_cells{idx,2} = mask_brain;
    end
end
