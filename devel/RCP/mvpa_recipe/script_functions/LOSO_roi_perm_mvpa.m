function predictions = LOSO_roi_perm_mvpa(subject_file_pointers,options)
...
for roi_idx = 1:numel(options.roi_list)
    ...
    for subject_idx = 1:numel(options.subjects)
        if ismember(options.subjects(subject_idx),options.exclusions) == 1
            ...
            switch options.trial_temporal_compression %termporal compression
                case 'on'
                    [lagged_data,subject_behavioral_data{subject_idx}] = temporal_compression(lagged_data,subject_behavioral_data{subject_idx},options);
            end
    ...
    for beh_idx = 1:numel(options.behavioral_file_list) %Cycle through valence levels, performing classification on each
        ...
        disp(message)
        txtappend(output_log,[datestr(now,31) ' ' message '\n']);
        true_labels = cell2mat(curr_behavioral_data); %store true labels
        
        for permidx = 1:options.num_perms2test
            if mod(permidx,10000) == 0
                message = sprintf('Permutation #%i/%i',permidx,options.num_perms2test);
                disp(message)
                txtappend(output_log,[datestr(now,31) ' ' message '\n']);
            end
            shuffle_order = randperm(numel(true_labels)); %permute order
            permd_labels = true_labels(shuffle_order);
            current_test = cell(numel(options.subjects),1);
            ...
            for idx = 1:numel(options.subjects)
                if ismember(options.subjects(idx),options.exclusions) == 1
                    ...
                    testing_labels = true_labels(testing_subject); %get true label for testing subject
                    training_labels = permd_labels(training_subjects'); %get permuted labels for training
                    ...
                    current_test{testing_subject} = cv_guesses;
                end
            end
