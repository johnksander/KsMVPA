function searchlight_cells = RSA_SL_enc2ret(options)
...
%   8.   test RDM against encoding RDMs
%   9.   store RDM fits 


%0. Initialize variables
%brain_cells = cell(numel(options.subjects),numel(options.roi_list)); %dropping this stuff
...
for roi_idx = 1:numel(options.roi_list)
    ...
    for subject_idx = 1:numel(options.subjects)
        if ismember(options.subjects(subject_idx),options.exclusions) == 0
            ...
            encodingRDMs = cellfun(@(x) x(mat2vec_mask),encodingRDMs,'Uniformoutput',false);
            encodingRDMs = cell2mat(encodingRDMs);
            ...
            parfor searchlight_idx = 1:numel(seed_inds)
                ...
                switch options.RDM_dist_metric
                    case 'spearman'
                        model_fit = corr(RDM,encodingRDMs,'type','Spearman');
                        model_fit = atanh(model_fit); %fisher Z transform
                    case 'kendall'
                        model_fit = kendall_tau([RDM,encodingRDMs]);
                        model_fit = diag(model_fit,1); %get the off diagonal value
                        model_fit = atanh(model_fit); %fisher Z transform
                end
                %   9. store RDM fits to encoding RDMs
