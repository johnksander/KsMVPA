function [brain_cells,searchlight_results] = xxRSA_SL_bigmem(options)


%07/10/2017: This script function is retired. Probably works fine, using it
...
for roi_idx = 1:numel(options.roi_list)
    ...
    searchlight_results = NaN(numel(seed_inds),2);
    %6. slice searchlights & assemble RDM
    parfor searchlight_idx = 1:numel(seed_inds) %parfor
        ...
        [r,p] = corr(RDM,Hmat,'type','Spearman','tail','right');
        searchlight_results(searchlight_idx,:) = [r p];
